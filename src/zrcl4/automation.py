...
def img_reg(token : 'AutoToken'):
    ...
    return locate(token.sourceImg, token.targetImg,_algo=token.cfg_imgAlgo, confidence=token.confidence)
...
def ocr(token : 'AutoToken'):
    ...
    if token.cfg_ocrMatchMethod == "fuzz":
        ...
        what = process.extract(
            token.text, res, scorer=process.fuzz.token_set_ratio
        )
        ...
        for item in res:
            if token.cfg_ocrMatchMethod == "exact" and item["text"] != token.text:
                continue
            elif token.cfg_ocrMatchMethod == "contains" and token.text not in item["text"]:
                continue
            elif token.cfg_ocrMatchMethod == "startswith" and not item["text"].startswith(token.text):
                continue
    ...
    # return as rect
    return (top_left[0], top_left[1], bottom_rght[0] - top_left[0], bottom_rght[1] - top_left[1])
...
class AutoToken:
    ...
    cfg_ocrMatchMethod : typing.Literal["startswith", "contains", "fuzz", "exact"] = "fuzz"
    ...
    def targetImg(self):
        ...
        ss = pyscreeze.screenshot(region=self.interestedRegion, allScreens=True if not self.wnd or self.wndInNonPrimaryMonitor else False)
    ...
    def monitorCenterPoint(self):
        ...
        return (self.monitor.x + self.monitor.width / 2, self.monitor.y + self.monitor.height / 2)
    ...
    def interestedRegion(self):
        ...
        match (self.wnd, self.monitor, self.region):
            ...
            case (wnd, _, region) if not self.regionIsRect:
                ...
                return (wnd.left + wnd.width / 2 - region[0] / 2, wnd.top + wnd.height / 2 - region[1] / 2, region[0], region[1])
            case (None, None, region) if not self.regionIsRect:
                self.monitor = get_primary_monitor()
                return (self.monitorCenterPoint[0] - region[0] / 2, self.monitorCenterPoint[1] - region[1] / 2, region[0], region[1])
            ...
            case (None, screeninfo.Monitor(_), region) if self.regionIsRect:
                return (self.monitorCenterPoint[0] - region[0] / 2, self.monitorCenterPoint[1] - region[1] / 2, region[2], region[3])
